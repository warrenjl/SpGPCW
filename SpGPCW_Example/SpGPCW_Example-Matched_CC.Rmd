...
for(k in 1:length(cases)){
  ...
  case_site <- site_id[cases[k]]
  
  eligible <- c(1:n)[(y == 0) & (site_id == case_site) & !used]
  
  if(length(eligible) == 0){
    stop(paste("No available control at site", case_site, "for case", cases[k]))
    }
  
  chosen <- eligible[sample(length(eligible), 1)]
  controls[k] <- chosen
  used[chosen] <- TRUE
  
  }

final_set <- c(cases, controls)
y <- y[final_set]
x <- x[final_set,]
z <- z[final_set,]
site_id <- site_id[final_set]
